...
int cautare(Pagina *pag, TipCheie x, int *contor)
{
    ...
    if (d == 0)
        return cautare(pag->p0, x, contor);
    return cautare(pag->e[d].p, x, contor);
}
...
int mare_key(Pagina *pag) {
    ...
    while (pag->e[pag->m].p != NULL) { // Parcurgem sub-arborii din dreapta
        pag = pag->e[pag->m].p;
    }
...
int next_mare(Pagina *pag, int x) {
    ...
    while (pag != NULL) {
        for (int i = 1; i <= pag->m; i++) {
            if (pag->e[i].cheie > x) {
                if (next == -1 || pag->e[i].cheie < next) {
                    next = pag->e[i].cheie;
                }
            }
        }
        // Trecem la urmatorul nod din arbore
        pag = pag->e[pag->m].p;
    }
    return next;
}
void afisare_recursiva(Pagina *arbore, int nivel) {
    ...
    //Afisam cheile din nodul curent
    printf("Nivel %d: ", nivel);
    for (int i = 1; i <= arbore->m; i++) {
        printf("%d ", arbore->e[i].cheie);
    }
    printf("\n");
    //Afisam subarborii stangi
    afisare_recursiva(arbore->p0, nivel + 1);
    //Afisam subarborii drepti
    for (int i = 1; i <= arbore->m + 1; i++) {
        afisare_recursiva(arbore->e[i].p, nivel + 1);
    }
}
int nr_key(Pagina *arbore) {
    ...
    //Numarul de chei din nodul curent
    int aux = arbore->m;
    //Adaugam numarul de chei din subarborele stange
    aux += nr_key(arbore->p0);
    //Adaugam numarul de chei din subarborele drept
    for (int i = 1; i <= arbore->m + 1; i++) {
        aux += nr_key(arbore->e[i].p);
    }
    return aux;
}
